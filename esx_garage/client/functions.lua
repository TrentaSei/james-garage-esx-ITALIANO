    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Auto: " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " Targa: " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Non hai parcheggiato la macchina qui."
            })
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Non possiedi nessuna auto."
            })
        end
...
VehicleAction = function(vehicleEntity, action)
    ...
    elseif action == "change_door_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Sei troppo lontano dalla tua auto per controllarla.") end
...
SpawnVehicle = function(vehicleProps)
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("~b~[Informazione]:~s~ Hai tirato fuori con successo la tua auto.")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("~b~[Informazione]:~s~ Hai parcheggiato con successo la tua auto.")
			else
				ESX.ShowNotification("~r~[Informazione]:~s~ Sei sicuro che questa auto sia la tua?")
			end
...
WaitForModel = function(model)
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Caricamento auto in corso " .. GetLabelText(GetDisplayNameFromVehicleModel(model)) .. "...", 255, 255, 255, 150)
	end
end